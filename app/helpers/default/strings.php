...
if (! function_exists('endsWith')) {
    ...
    function endsWith($haystack, $needles)
    {
        foreach ((array) $needles as $needle) {
            if ((string) $needle === usubstr($haystack, -length($needle))) {
                return true;
            }
        }
...
if (! function_exists('str_random')) {
    ...
    function str_random($length = 16)
    {
        ...
        while (($len = strlen($string)) < $length) {
            ...
            $string .= usubstr(str_replace(['/', '+', '='], '', base64_encode($bytes)), 0, $size);
        }
...
if (! function_exists('url')) {
    ...
    function url($url = '')
    {
        $full_url = env('APP_URL', 'http://localhost');
...
if (! function_exists('usubstr')) {
     ...
     * @return string
     */
    function usubstr($string, $start, $length = null)
    {
